...
const clinicalItems = [
    { name: "Clinical Worklist", icon: "medical_services", href: "/clinical/worklist", iconColor: "text-blue-600", iconBg: "bg-blue-100" },
    { name: "Review Case", icon: "rate_review", href: "/clinical/review", iconColor: "text-emerald-600", iconBg: "bg-emerald-100" },
    { name: "Clinical History", icon: "history", href: "/clinical/history", iconColor: "text-violet-600", iconBg: "bg-violet-100" },
];
...
export default function DoctorSidebar() {
    ...
    const renderNav = (items: typeof clinicalItems) =>
        items.map((item) => {
    ...
    return (
        <aside className="w-64 bg-white border-r border-slate-200/80 fixed h-[calc(100vh-64px)] overflow-y-auto hidden lg:flex flex-col">
            <div className="flex-1 p-5">
                <p className="text-[11px] font-extrabold text-slate-400 uppercase tracking-[0.15em] mb-3 px-1">Clinical Validation</p>
                <nav className="space-y-1 mb-8">{renderNav(clinicalItems)}</nav>
            </div>
