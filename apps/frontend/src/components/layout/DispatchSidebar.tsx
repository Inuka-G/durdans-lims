...
const dispatchItems = [
    { name: "Dispatch Dashboard", icon: "dashboard", href: "/dispatch/dashboard", iconColor: "text-blue-600", iconBg: "bg-blue-100" },
    { name: "Delivery Status", icon: "local_shipping", href: "/dispatch/delivery-status", iconColor: "text-emerald-600", iconBg: "bg-emerald-100" },
    { name: "Failed Deliveries", icon: "error_outline", href: "/dispatch/failed-deliveries", iconColor: "text-rose-600", iconBg: "bg-rose-100" },
    { name: "Authorized Reports", icon: "verified", href: "/dispatch/authorized-reports", iconColor: "text-violet-600", iconBg: "bg-violet-100" },
];
...
export default function DispatchSidebar() {
    ...
    const renderNav = (items: typeof dispatchItems) =>
        items.map((item) => {
    ...
    return (
        <aside className="w-64 bg-white border-r border-slate-200/80 fixed h-[calc(100vh-64px)] overflow-y-auto hidden lg:flex flex-col">
            <div className="flex-1 p-5">
                <p className="text-[11px] font-extrabold text-slate-400 uppercase tracking-[0.15em] mb-3 px-1">Report Dispatch</p>
                <nav className="space-y-1 mb-8">{renderNav(dispatchItems)}</nav>
            </div>
