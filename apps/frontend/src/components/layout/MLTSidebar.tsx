...
const testingItems = [
    { name: "Sample Worklist", icon: "science", href: "/mlt/worklist", iconColor: "text-blue-600", iconBg: "bg-blue-100" },
    { name: "All Worklist", icon: "list_alt", href: "/mlt/all-worklist", iconColor: "text-sky-600", iconBg: "bg-sky-100" },
    { name: "Result Entry", icon: "edit_note", href: "/mlt/result-entry", iconColor: "text-emerald-600", iconBg: "bg-emerald-100" },
];

const toolsItems = [
    { name: "QC Dashboard", icon: "assessment", href: "/mlt/qc-dashboard", iconColor: "text-violet-600", iconBg: "bg-violet-100" },
    { name: "Instruments", icon: "biotech", href: "/mlt/instruments", iconColor: "text-amber-600", iconBg: "bg-amber-100" },
];
...
export default function MLTSidebar() {
    ...
    const renderNav = (items: typeof testingItems) =>
        items.map((item) => {
    ...
    return (
        <aside className="w-64 bg-white border-r border-slate-200/80 fixed h-[calc(100vh-64px)] overflow-y-auto hidden lg:flex flex-col">
            <div className="flex-1 p-5">
                <p className="text-[11px] font-extrabold text-slate-400 uppercase tracking-[0.15em] mb-3 px-1">MLT Testing</p>
                <nav className="space-y-1 mb-8">{renderNav(testingItems)}</nav>

                <p className="text-[11px] font-extrabold text-slate-400 uppercase tracking-[0.15em] mb-3 px-1">Tools</p>
                <nav className="space-y-1">{renderNav(toolsItems)}</nav>
            </div>
